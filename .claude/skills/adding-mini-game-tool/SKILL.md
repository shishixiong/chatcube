...
| 6 | `components/XxxCard.ets` (NEW) | Define `XxxResult` interface, `@ComponentV2 struct XxxCard` with `@Param toolCall/isAnswered/answeredPayload/largeSize`, `@Event onAnswer` | Build succeeds |
| 7 | `components/MessageBubble.ets` (L20, L39, ~L2140, L2580, L2632, L2642, ~L2747, L3102, L3531, ~L3967) | Import, inline render, 3 or-chain extensions, helper, click dispatch, step switch, step content builder | `grep -c "isXxxFunctionName" MessageBubble.ets` ≥ 6 |
...
**Run all 4 greps after step 4a:**

```bash
grep -n "META_XXX\|META_PUZZLE\|META_MATH\|META_ENGLISH\|META_HANDWRITING" entry/src/main/ets/models/StarEventModels.ets   # expect new META_XXX
grep -n "byType\." entry/src/main/ets/models/StarEventModels.ets                                                       # expect byType.xxx in 3+ places
grep -n "return META_" entry/src/main/ets/models/StarEventModels.ets                                                   # expect new return META_XXX
grep -n "return byType\." entry/src/main/ets/models/StarEventModels.ets                                                # expect new return byType.xxx
```
